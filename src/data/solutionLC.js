export const LeetCodeSolutions = [
  ...
  "Go": "package main; import \"fmt\"; func twoSum(nums []int, target int) []int { complementMap := make(map[int]int); for i, num := range nums { complement := target - num; if index, exists := complementMap[complement]; exists { return []int{index, i}; } complementMap[num] = i; } return nil; } func main() { nums := []int{2, 7, 11, 15}; target := 9; result := twoSum(nums, target); if result != nil { fmt.Printf(\"Indices: %v\\n\", result); } else { fmt.Println(\"No solution found!\"); } }",
  'Time_Complexity': "O(n)",
